...
class pdqn(tf.keras.Model):
    ...
    def set_model_weights(self,sdir):
        tar_count_cnn = len([key for key in self.layer_dict.keys() if "Tar_cnn_layer" in key])
        tra_count_cnn = len([key for key in self.layer_dict.keys() if "Tra_cnn_layer" in key])
        tar_count_dense = len([key for key in self.layer_dict.keys() if "Tar_dense" in key])
        tra_count_dense = len([key for key in self.layer_dict.keys() if "Tra_dense" in key])
        ...
        for i in range(0,tar_count_dense+1):
            if i != 0:
                dense = "dense_"+str(i)
            layer_key = "Tar_dense"+str(i) if i < tar_count_dense else "Tar_fdense"
        ...
        for i in range(0,tra_count_dense+1):
            if i != 0:
                dense = "dense_"+str(i)
            layer_key = "Tra_dense"+str(i) if i < tra_count_dense else "Tra_fdense"                
            self.layer_dict[layer_key].set_weights([
                f[dense]['pdqn'][dense]['kernel:0'][()],
                f[dense]['pdqn'][dense]['bias:0'][()]
            ])
    ...
    def call(self, inputs,training=False):
        ...
        y_scr = tf.scatter_nd_update(tf.Variable(Tra_out),indices=update_idx,updates=y)
