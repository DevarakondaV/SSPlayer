...
class experience:
    ...
    def sample(self, batch_size):
        ...
        if (self.total_exp >= self.tree.capacity):
            min_prob = np.min(
                self.tree.tree[-self.tree.capacity:]) / self.tree.get_total_priority()
        else:
            min_prob = np.min(self.tree.tree[-self.tree.capacity:self.tree.capacity -
                                             1+self.tree.data_i]) / self.tree.get_total_priority()
        ...
        for i in range(0, batch_size):
            ...
            try:
                # .astype(np.float16)[0]
                rand_p = np.random.uniform(p_seg*i, p_seg*(i+1), 1)
            ...
            try:
                ...
                leaf_idx, priority_i, exp_i = self.tree.get_leaf(
                    rand_p, p=True)
                print(rand_p)
