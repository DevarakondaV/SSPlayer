...
class Trainer:
    ...
    def __init__(self, log):
        ...
        args:
            ...
            max_exp_len:        int. Maximum length of experience vector
            min_exp_len_train:  int. Minimum length of exp before training
        ...
        # Params used while training
        self.game_play_iteration = 0  # Number of iterations of game play
        self.num_train_ops = 0  # Number of training operations
        self.log = log
        self.pause = False
    ...
    def process_seq(self, seq, seq_len):
        """
            ...
            args:
                seq: List. Containing SARSA for game iteration
        ...
        if (len(seq) < seq_len):
            frames = [np.zeros(shape=[84, 84, 1]).astype(np.uint8)
                      for i in range(0, seq_len-len(seq))] + [i for i in seq]
        else:
            frames = seq
    ...
    def create_reward_plot(self, r_iter, save_dir):
        """
        Functions creates a plot of the reward vs game iteration.

        args:
            g_iter: int. NNumber of total games played
            r_iter: List of ints. List containing reward for each game iteration
    ...
    def play_train(self, net, game, learning_rate,
        ...
        for i in range(0, run_times):
            ...
            if not self.force_kill:
                self.train(net, game, n, seq_len, batch_size,
                           greed_frames, max_exp_len_train, learning_rate, i)
                net.save_weights(weights_save_dir +
                                 "weights{}.h5".format(i), save_format='h5')
            ...
            while current_tran < n:
                ...
                while not game.stop_play:
                    ...
                    if (len(self.exp) > batch_size and len(self.exp) > 10000):
                        leaf_idx, IS_weights, seq_n = self.exp.sample(
                            batch_size)
                        IS_weights = np.reshape(IS_weights, (batch_size, 1))
                        y, Tra_d3 = net.train(seq_n, IS_weights, r)
                        self.update_exp(leaf_idx, np.amax(
                            (y-Tra_d3).numpy(), axis=1))
    ...
    def play(self, net, game, seq_len, num_times, TSNE=False, TSNE_size=0):
        ...
        if (TSNE):
            tarmmep = np.memmap("tsne/tar", dtype=np.float16,
                                mode='w+', shape=(TSNE_size, 512))
            vmmep = np.memmap("tsne/v", dtype=np.float16,
                              mode='w+', shape=(TSNE_size,))
            ammep = np.memmap("tsne/a", dtype=np.float16,
                              mode='w+', shape=(TSNE_size,))
            ...
            for i in range(0, num_times):
                ...
                while not game.stop_play:
                    ...
                    if not TSNE:
                        ...
                        vmmep[t_i:] = np.amax(
                            Tar_d3.numpy()).astype(np.float16)
